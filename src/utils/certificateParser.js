...
async function loadWasmModule() {
  ...
  if (typeof process !== 'undefined' && process.versions && process.versions.node) {
    // Node.js environment - use CommonJS build from pkg/ (auto-initializes)
    wasm = await import('../../cert-parser-wasm/pkg/cert_parser_wasm.js');
    wasmInitialized = true; // Node.js build self-initializes
  } else {
    // Browser environment - use ES module build from pkg-web/
    const wasmModule = await import('../../cert-parser-wasm/pkg-web/cert_parser_wasm.js');
...
// Direct PEM parsing for tests (doesn't require FileReader)
export async function parsePEM(pemData) {
  await ensureWasmInit();
  
  const result = wasm.parse_pem(pemData);
  return processWasmResult(result, 'pem-data');
}

// Direct DER parsing for tests (doesn't require FileReader)
export async function parseDER(derData) {
  await ensureWasmInit();
  
  const uint8Array = derData instanceof Uint8Array ? derData : new Uint8Array(derData);
  const result = wasm.parse_der(uint8Array);
  return processWasmResult(result, 'der-data');
}
